...
$(document).ready(function(){
	...
	$("#cmdChangeWorkMode").click(function(){
		$(this).attr("disabled");
		changeWorkMode();
	});
...
function loadData(){
	$.get("/api/v1/get_data", function(json){
		...
		if(cfg["work_mode"] == 0){
			$("#ckBridgeMode").attr("checked","checked");
		}
		else{
			$("#ckBridgeMode").removeAttr("checked");
			$("#ckStandaloneMode").attr("checked","checked");
		}
	});
};
...
function changeWorkMode(){
	var mode = $("#sta-setting-page input[name='cmdWorkingMode']:checked").val();
	$.post("/api/v1/set_mode", {work_mode: mode}, function(json){
		if(json["success"]){
		...
		$("#cmdWorkingMode").removeAttr("disabled");
	});
};
