...
bool wifi_sta::checkInternet()
{
    ...
    http_request::get("http://farmapi.agrhub.com/location/get", "", [](const char* resp, size_t len){
...
bool wifi_sta::connectSTA(char* ssid, char* pwd, uint16_t timeout)
{
    ...
    esp_err_t err = tcpip_adapter_set_hostname(TCPIP_ADAPTER_IF_STA, "agrhub.local");
    if (err){        
...
uint64_t wifi_sta::getTimeStamp()
{
    ...
    if(millis < SOFTWARE_DATE && connected == true)
    {
        initialize_sntp();
    }
    return millis;
